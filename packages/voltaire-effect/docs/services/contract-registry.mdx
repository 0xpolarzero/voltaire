...
const AppLayer = Contracts.layer.pipe(Layer.provide(ProviderLayer))
...
const program = Effect.gen(function* () {
  const { Token } = yield* Contracts.Service
...
Types are preserved automatically — `yield* Contracts.Service` returns a fully typed map matching your registry config. No type helpers needed.
...
const ContractsLayer = isMainnet ? mainnetContracts.layer : arbitrumContracts.layer
```
...
- [Block Explorer API](/services/block-explorer-api) - ABI resolution from Sourcify/Etherscan/Blockscout
